...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        # delete meta_task_entity in entity set
    ...
    def load_pretrain_embedding(self):
        ...
        if self.args.pre_train:
            ...
            if self.args.pre_train_model == 'KG2E':
                self.entity_embedding = torch.Tensor(np.load('./embed/{}/entityEmbedding.npy'.format(self.args.data)))
                self.entity_covar = torch.Tensor(np.load('./embed/{}/entityCovar.npy'.format(self.args.data)))
                self.relation_embedding = torch.Tensor(np.load('./embed/{}/relationEmbedding.npy'.format(self.args.data)))
                self.relation_covar = torch.Tensor(np.load('./embed/{}/relationCovar.npy'.format(self.args.data)))
                self.pretrain_entity_embedding = torch.cat([self.entity_embedding, self.entity_covar], 0)
                self.pretrain_relation_embedding = torch.cat([self.relation_embedding, self.relation_covar], 0)
    ...
    def load_model(self):
        ...
        if self.args.model == 'InducGEN':
            ...
            # print("in trainer_induc.py", self.pretrain_entity_embedding.shape, self.pretrain_relation_embedding.shape)
            # print("model args: ", self.model.entity_embedding.weight.shape, self.model.relation_embedding.shape)
        else:
    ...
    def train(self):
        ...
        for epoch in trange(0, (self.args.n_epochs + 1), desc = 'Train Epochs', position = 0):
            ...
            # print(self.args.num_train_entity)
            # print("total loss", total_loss)
            ...
            mrr_file = './checkpoints/{}/mrr.dat'.format(self.exp_name)
            hits1_file = './checkpoints/{}/hits1.dat'.format(self.exp_name)
            hits3_file = './checkpoints/{}/hits3.dat'.format(self.exp_name)
            hit10_file = './checkpoints/{}/hits10.dat'.format(self.exp_name)
            ...
            if epoch % self.args.evaluate_every == 0:
                ...
                if epoch == 0:
                    with open(mrr_file, 'w') as f:
                        f.write('{}\n'.format(results['total_mrr']))
                    with open(hits1_file, 'w') as f:
                        f.write('{}\n'.format(results['total_hits@1']))
                    with open(hits3_file, 'w') as f:
                        f.write('{}\n'.format(results['total_hits@3']))
                    with open(hit10_file, 'w') as f:
                        f.write('{}\n'.format(results['total_hits@10']))
                else:
                    with open(mrr_file, 'a') as f:
                        f.write('{}\n'.format(results['total_mrr']))
                    with open(hits1_file, 'a') as f:
                        f.write('{}\n'.format(results['total_hits@1']))
                    with open(hits3_file, 'a') as f:
                        f.write('{}\n'.format(results['total_hits@3']))
                    with open(hit10_file, 'a') as f:
                        f.write('{}\n'.format(results['total_hits@10']))
    ...
    def eval(self, eval_type='test'):
        ...
        for unseen_entity in test_task_pool:
            ...
            if self.args.pre_train_model == 'KG2E':
                ranks, ranks_s, ranks_o = utils.calc_induc_mrr(unseen_entity, unseen_entity_embedding,
                                                               self.model.entity_embedding.weight,
                                                               self.model.relation_embedding, test_triplets,
                                                               self.all_triplets, self.use_cuda,
                                                               score_function=self.args.score_function,
                                                               all_relation_var=self.model.relation_covar,
                                                               all_entity_var=self.model.relation_covar)
            else:
