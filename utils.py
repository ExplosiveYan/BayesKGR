...
# return ranks
def calc_induc_mrr(unseen_entity, unseen_entity_embedding, all_entity_embeddings, all_relation_embeddings,
                   test_triplets, all_triplets, use_cuda, score_function='DistMult', all_relation_var=None, all_entity_var=None):
    ...
    for test_triplet in test_triplets:
        ...
        if test_triplet[0] == unseen_entity:
            ...
            if score_function == 'DistMult':
                ...
                # print(unseen_entity_embedding)
                head_embedding = unseen_entity_embedding[:dimension]
                head_covar = unseen_entity_embedding[dimension:]
                relation_embedding = all_relation_embeddings[relation]
                relation_covar = all_relation_var[relation]
                tail_embedding = all_entity_embeddings[perturb_entity_index]
                tail_covar = all_relation_var[perturb_entity_index]
                # print(head_embedding, tail_embedding)
                errorm = tail_embedding - head_embedding
                errorv = tail_covar - head_covar
                # print(errorv, relation_covar)
                # print(head_embedding.shape, head_covar.shape, relation_embedding.shape, relation_covar.shape, tail_embedding.shape, tail_covar.shape)
                ...
                def KLScore(**kwargs):
                    ...
                    KLer = (losep1 + losep2 - dimension) / 2
                    # print(KLer)
                    ...
                    KLre = (losep1 + losep2 - dimension) / 2
                    # print(KLre)
                    return (KLer + KLre) / 2

                score = KLScore(relationm=relation_embedding, relationv=relation_covar,
                                errorm=errorm, errorv=errorv)
            ...
            if score_function == 'DistMult':
                ...
                head_embeddings = all_entity_embeddings[perturb_entity_index]
                head_covar = all_entity_var[perturb_entity_index]
                relation_embedding = all_relation_embeddings[relation]
                relation_covar = all_relation_var[relation]
                tail_embedding = unseen_entity_embedding[:dimension]
                tail_covar = unseen_entity_embedding[dimension:]
                errorm = tail_embedding - head_embeddings
                errorv = tail_covar - head_covar
                score = KLScore(relationm=relation_embedding, relationv=relation_covar,
                                errorm=errorm, errorv=errorv)
...
def cal_trans_mrr(unseen_entity, unseen_entity_embedding, all_entity_embeddings, all_relation_embeddings,
                  test_triplets_dict, all_triplets, use_cuda, score_function='DistMult'):
    num_entity = len(all_entity_embeddings)
    subject_count = 0
    object_count = 0
